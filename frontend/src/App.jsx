...
function App () {
  ...
  const handleAuth = e => {
    ...
    // 1. Rejimni aniq tekshiramiz (authMode holatiga qarab endpoint tanlanadi)
    ...
    fetch(`${API_URL}/${endpoint}`, {
      ...
      .then(async res => {
        // Agar backend xato (400, 500) qaytarsa, xato matnini backenddan o'qiymiz
        if (!res.ok) {
          const errorData = await res.json().catch(() => ({}))
          throw new Error(errorData.message || 'Tizim xatosi!')
        }
        return res.json()
      })
      .then(data => {
        if (endpoint === 'login' && data.token) {
          // MUVAFFAQIYATLI LOGIN: Token saqlanadi va profil ochiladi
          ...
          // MUVAFFAQIYATLI REGISTRATSIYA: Foydalanuvchiga xabar beramiz va loginga o'tkazamiz
          alert(
            data.message ||
              'Foydalanuvchi muvaffaqiyatli yaratildi! Endi kirish sahifasidan tizimga kiring.'
          )
          setAuthMode('login') // Rejimni Login-ga o'tkazish
          setFormUser('')
          setFormPass('') // Inputlarni tozalash
        }
      })
      .catch(err => {
        console.error(err)
        ...
        {token ? (
          <div className='flex items-center gap-4'>
            ...
            <button
              ...
              className='bg-rose-500 hover:bg-rose-600 text-white text-sm font-semibold px-4 py-2 rounded-xl transition shadow-sm'
            >
              Chiqish
            </button>
          </div>
          ...
          <p className='text-sm text-gray-400 font-medium'>
            Mahsulot qo'shish yoki o'chirish uchun tizimga kiring
          </p>
        )}
      </header>
      ...
      <main className='max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-3 gap-8'>
            ...
            <div>
              ...
              <form onSubmit={handleAuth} className='space-y-3'>
                <input
                  ...
                  className='w-full border border-gray-200 p-3 rounded-xl focus:outline-none focus:border-blue-500 transition text-sm'
                  required
                />
                <input
                  type='password'
                ...
                <button
                  type='submit'
                  className='w-full bg-blue-600 text-white p-3 rounded-xl hover:bg-blue-700 transition font-bold text-sm shadow-md shadow-blue-100'
                >
                  {authMode === 'login' ? 'Tizimga Kirish' : 'Hisob Yaratish'}
                </button>
              </form>
              {/* Buni pastroqdagi formaning tagidagi <p> elementi bilan solishtiring */}
              <p
                onClick={() =>
                  setAuthMode(prev => (prev === 'login' ? 'register' : 'login'))
                }
                className='text-xs text-blue-500 text-center mt-4 cursor-pointer hover:underline font-medium'
              >
                {authMode === 'login'
                  ? 'Yangi hisob yaratish'
                  : 'Menda hisob bor, kirish sahifasi'}
              </p>
            </div>
          ) : (
            <div>
              ...
              <form onSubmit={handleAddProduct} className='space-y-3'>
                ...
                <input
                  type='text'
                  placeholder='Kategoriya (masalan: Elektronika)'
                  value={category}
                ...
                <button
                  type='submit'
                  className='w-full bg-emerald-500 text-white p-3 rounded-xl hover:bg-emerald-600 transition font-bold text-sm shadow-md shadow-emerald-100'
                >
                  Omborga Joylash
                </button>
              </form>
        ...
        <div className='lg:col-span-2'>
          <h2 className='text-2xl font-black text-gray-900 mb-6'>
            Sotuvdagi Mahsulotlar Ro'yxati
          </h2>
          {products.length === 0 ? (
            <p className='text-gray-400 bg-white p-6 rounded-2xl border text-center font-medium'>
              Hozircha mahsulotlar yo'q. Birinchilardan bo'lib qo'shing!
            </p>
          ) : (
            <div className='grid grid-cols-1 sm:grid-cols-2 gap-4'>
              {products.map(product => (
                <div
                  key={product._id}
                  className='bg-white p-5 rounded-2xl shadow-sm border border-gray-100 flex flex-col justify-between hover:shadow-md transition group'
                >
                  <div>
                    <span className='text-[10px] font-bold uppercase tracking-widest text-blue-600 bg-blue-50 px-2.5 py-1 rounded-md'>
                      {product.category}
                    </span>
                    <h3 className='text-lg font-bold text-gray-800 mt-2 group-hover:text-blue-600 transition'>
                      {product.name}
                    </h3>
                  ...
                  {token && (
                    <button
                      ...
                      className='mt-5 w-full bg-rose-50 text-rose-600 font-bold p-2.5 rounded-xl hover:bg-rose-500 hover:text-white transition text-center text-xs'
                    >
